...
For the purposes of this analysis, all R-code is shown along with the output.
...
{
  if (class(data.raw[,i]) == "factor") levels(data.raw[,i]) <- gsub("\n","", levels(data.raw[,i]))
}
...
The dataset was evaluated for missing data.  Several attributes are considered critical to the analysis, victim attributes and relationship of victim to offender; observations with insuffient basic information were removed.
...
This results in the removal of `r nrow(data.raw)-nrow(data)` lines.  For the other attributes, the number of missing fields was determined to help determine the best approach for handling them.  
...
We looked at the geographic distribution of incidents in the data.  The incidents are generally spread across the US, although there are no examples from the northeast or the northwest states.  The majority of incidents occur in the south and central regions.
...
The correlation of several key attributes is shown below.
...
```
 ...
 Interestingly from a data perspective, rural.city is correlated to victim gender and publicized; rural incidentsin our sample data set are more likely to be publicized, incidents in cities are more often male victims.  Victim age and offender gender are slighlty correlated to victim-offender relationship.  
...
p2 <- ggplot(data, aes(x=Rural.City, fill=Publicized)) + geom_bar(stat='bin') + ggtitle("Publicized and Rural/City")
p3 <- ggplot(data, aes(x=Victim.AgeGroup, fill=Relate.Group)) + geom_bar(stat='bin') + ggtitle("Victim Age and Relationship to Offender")
p4 <- ggplot(data, aes(x=Offender.Gender, fill=Relate.Group)) + geom_bar(stat='bin') + ggtitle("Offender Gender and Relationship to Victim")
...
p1 <- ggplot(data, aes(x=Victim.Gender, fill=as.factor(target))) + geom_bar(stat='bin') + ggtitle("Violence and Victim Gender")
p2 <- ggplot(data, aes(x=Victim.AgeGroup, fill=as.factor(target))) + geom_bar(stat='bin') + ggtitle("Violence and Victim Age")
grid.arrange(p1,p2, ncol=2)

```


The race of the offender and victim is highly correlated, which is unsurpising.  Looking further into the data, White and Black offenders dominate abduction cases with whites dominating in Southeast, Central and blacks dominating in South Central and West, hispanic abduction appears rare in our data from our additional research with DCFS this is may be due to non reporting and immigration status.  OUr FBI sponsor has cautioned about including race and region in the analysis as this are politcally sensitive topics, however from a data driven perspective we have maintained them for the analysis.

```{r warning=FALSE}
p1 <- ggplot(data, aes(x=Offender.Race, fill=Victim.Race)) + geom_bar(stat='bin') + ggtitle("Victim and Offender Race")
p2 <- ggplot(data, aes(x=Region, fill=Victim.Race)) + geom_histogram(binwidth=.5, position="dodge") + ggtitle("Victim Race by Region")
grid.arrange(p1,p2, ncol=2)
```
...
We see higher counts in the Souteast and South Central, as we saw in the geographic plot, and the father appears to be the most common offender across all regions.
...
The majority of cases are pubicized with the highest being in the South Central Region.  With the small data set, its difficult to drawn conclusions.  If this holds in the final data set, additional sponsor knowledge will be sought to understand why this is the case and the interaction of publicity with the trand toward violence.
...
Due to the small sample size, bootstrap sampling was used, which uses random sampling with replacement to create the training data.  With the extrenely small size of our sample data set, even with bootstrap sampling, the results of different runs tend to be variable.  Final interpreations and conclusions on the modeling will be done once we get teh full data set next week.
...
Typically, the top performers are svm.v2, naiveBayes, randomForest.v6.  That 200 repetition bootstrap still gives varying results is indicative the the sample size is insufficient.  With 20 observations and 10 factors, the models are undoubtedly overfitting.
...
Once we get the additional data, we will quickly run through the analysis and share the results with the sponsor.  This preliminary analysis has uncovered a number of interesting questions that can only be answered with a larger data set.  We also expect to receive much larger data sets from National Center for Missing and Exploited Children, NCMEC, which will need to be processed and determine if similar modeling is applicable.  With all data in hand, we will re-evalute our choice of target variable and model techniques in conversations with our sponsor and stakeholders.
